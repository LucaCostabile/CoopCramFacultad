...
export default function Account() {
  ...
  async function loadOrders(nextPage, replace = false) {
    try {
      ...
      const r = await api.get("/account/orders", {
  ...
  async function onSave(e) {
    ...
    setError("");
    try {
      const r = await api.patch("/account", {
        email: email || null,
        phone: phone || null,
      });
      setUser(r.data);
  ...
  async function toggleItems(orderId) {
    const el = document.querySelector(
      `[data-order-id="${orderId}"] .pedido-items`
    );
    if (!el) return;
    ...
    <div className="container">
      <main className="main-cuenta">
        <section className="formulario usuario">
          ...
          {error && (
            <div className="error-msg" role="alert" style={{ marginTop: 6 }}>
              {error}
            </div>
          )}
          <form onSubmit={onSave}>
            ...
            >
              ...
              <div>
                ...
                {!email && (
                  <div
                    style={{ color: "#b91c1c", fontWeight: 700, marginTop: 6 }}
                  >
                    Recomendado: agregá tu correo para recuperar la contraseña.
                  </div>
                )}
              </div>
              <div>
                ...
                {!phone && (
                  <div
                    style={{ color: "#b91c1c", fontWeight: 700, marginTop: 6 }}
                  >
                    Recomendado: agregá un teléfono de contacto.
                  </div>
                )}
              </div>
              <div>
            ...
            <div
              className="login-actions solo-boton"
              style={{
                display: "flex",
                gap: 10,
                alignItems: "center",
        ...
        <section className="formulario pedidos" style={{ marginTop: 32 }}>
          ...
          <div id="orders-list" className="lista-pedidos">
            ...
            {orders.map((o) => (
              ...
              >
                ...
                >
                  <div>
                    <div className="pedido-title" style={{ fontWeight: 700 }}>
                      Pedido #{o.id}
                    </div>
                  ...
                  >
                    <div>
                      <strong>Items:</strong> {o.items_count}
                    </div>
                    {o.total != null && (
                      <div>
                        <strong>Total:</strong> $
                    ...
                    <button
                      type="button"
                      className="btn-outline btn-sm btn-toggle-items"
          ...
          >
            {hasMore && (
              ...
              >
                {loadingOrders ? "Cargando..." : "Ver más"}
              </button>
            )}
          </div>
        </section>
